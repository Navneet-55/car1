...
class Camera {
    ...
    private let chaseSmoothing: Float = 4.0 // Smoothing factor
    private let targetSmoothing: Float = 6.0
    ...
    private let fovSmoothing: Float = 3.0 // FOV smoothing factor
    ...
    /// Dynamic chase camera with speed-based distance scaling and frame-rate independent smoothing
    private func updateChaseCamera(targetPosition: SIMD3<Float>, targetRotation: simd_quatf, deltaTime: Float) {
        ...
        smoothPosition = mix(smoothPosition, desiredPosition, t: 1.0 - exp(-chaseSmoothing * deltaTime))
        ...
        smoothTarget = mix(smoothTarget, lookAheadTarget, t: 1.0 - exp(-targetSmoothing * deltaTime))
        ...
        smoothFov = mix(smoothFov, targetFov, t: 1.0 - exp(-fovSmoothing * deltaTime))
        fov = smoothFov
    }
    ...
    private func updateCameraShake(deltaTime: Float) {
        ...
        // Decay shake (frame-rate independent)
        shake.intensity *= exp(-shake.decay * deltaTime)
    }
